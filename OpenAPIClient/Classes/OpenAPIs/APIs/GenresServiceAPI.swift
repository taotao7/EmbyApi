...
open class GenresServiceAPI {
     ...
     */
    @discardableResult
    open class func getGenresByName(name: String, userId: String? = nil, apiResponseQueue: DispatchQueue = OpenAPIClientAPI.apiResponseQueue, completion: @escaping ((_ data: BaseItemDto?, _ error: Error?) -> Void)) -> RequestTask {
        return getGenresByNameWithRequestBuilder(name: name, userId: userId).execute(apiResponseQueue) { result in
            switch result {
            case let .success(response):
                completion(response.body, nil)
            case let .failure(error):
    ...
    open class func getGenresByNameWithRequestBuilder(name: String, userId: String? = nil) -> RequestBuilder<BaseItemDto> {
        var localVariablePath = "/Genres/{Name}"
