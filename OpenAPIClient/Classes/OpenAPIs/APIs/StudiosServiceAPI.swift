...
open class StudiosServiceAPI {
     ...
     */
    @discardableResult
    open class func getStudiosByName(name: String, userId: String? = nil, apiResponseQueue: DispatchQueue = OpenAPIClientAPI.apiResponseQueue, completion: @escaping ((_ data: BaseItemDto?, _ error: Error?) -> Void)) -> RequestTask {
        return getStudiosByNameWithRequestBuilder(name: name, userId: userId).execute(apiResponseQueue) { result in
            switch result {
            case let .success(response):
                completion(response.body, nil)
            case let .failure(error):
    ...
    open class func getStudiosByNameWithRequestBuilder(name: String, userId: String? = nil) -> RequestBuilder<BaseItemDto> {
        var localVariablePath = "/Studios/{Name}"
