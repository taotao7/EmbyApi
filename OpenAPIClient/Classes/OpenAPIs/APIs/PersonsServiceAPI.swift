...
open class PersonsServiceAPI {
     ...
     */
    @discardableResult
    open class func getPersonsByName(name: String, userId: String? = nil, apiResponseQueue: DispatchQueue = OpenAPIClientAPI.apiResponseQueue, completion: @escaping ((_ data: BaseItemDto?, _ error: Error?) -> Void)) -> RequestTask {
        return getPersonsByNameWithRequestBuilder(name: name, userId: userId).execute(apiResponseQueue) { result in
            switch result {
            case let .success(response):
                completion(response.body, nil)
            case let .failure(error):
    ...
    open class func getPersonsByNameWithRequestBuilder(name: String, userId: String? = nil) -> RequestBuilder<BaseItemDto> {
        var localVariablePath = "/Persons/{Name}"
