...
let name = "name_example" // String | The person name
let userId = "userId_example" // String | Optional. Filter by user id, and attach user data (optional)

// Gets a person, by name
PersonsServiceAPI.getPersonsByName(name: name, userId: userId) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
